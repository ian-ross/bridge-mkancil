...
# ==============================
  ...
  proc dispatch {w cmd args} \
  {
    ...
    switch -glob -- $cmd \
    {
      cge*    { uplevel 1 [linsert $args 0 ::scrolledframe::cget   $w] }
  ...
  # cget operation
  #
  # get configure info from the widget
  # --------------
  # parm1: widget name
  # parm2: options
  # --------------
  proc cget {w args} \
  {
    ...
    switch -glob -- $args \
    {
      -fill   \
      {
        if {$($w:fillx) == 0 && $($w:filly) == 0} {
           set result none
        } elseif {$($w:fillx) == 1 && $($w:filly) == 0} {
           set result x
        } elseif {$($w:fillx) == 0 && $($w:filly) == 1} {
           set result y
        } elseif {$($w:fillx) == 1 && $($w:filly) == 1} {
           set result both
        }
        set flag 1
      }
      -xsc*   \
      {
        set result $($w:xscroll)
        set flag 1
      }
      -ysc*   \
      {
        set result $($w:yscroll)
        set flag 1
      }
      default { lappend options $args }
    }
    # check if needed
    if {!$flag || $options != ""} \
    {
      # call frame cget
      set result [uplevel 1 [linsert $options 0 ::scrolledframe::_$w cget]]
    }
